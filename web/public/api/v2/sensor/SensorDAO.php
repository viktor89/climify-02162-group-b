...
class SensorDAO extends API\V2\Api
{
    ...
    public function getSensorInstance(){
        $statement = $this->database->prepare("SELECT SensorID, SensorTypeID, HubID, RoomName,BuildingID FROM Room NATURAL JOIN SensorInstance ");
        ...
        while ($statement->fetch()) {
            $hubs[] = ["PiID" => $hubMac, "building" => $building, "room" => $room, "SensorType"=>$stype, "SensorID" => $sID ];
        }
        $statement->close();
